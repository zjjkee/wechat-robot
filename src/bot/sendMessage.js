...
import { botName, roomWhiteList, aliasWhiteList } from '../../config/config.js'
...
export async function defaultMessage(msg, bot) {
  ...
  const isRoom = Boolean(room) && content.includes(`@${botName}`) // 群聊，且包含botname
  ...
  if (isText && !isBotSelf) {
    ...
    try {
      ...
      if (isRoom) {;
        console.log('。。。开始回复群聊');
        await room.say(ms,contact)
        return
      }
      // 回复（白名单）的私人消息
      if (isAlias && !room) {
        await contact.say(ms);
      }
    } catch (e) {
      console.error(e)
    }
